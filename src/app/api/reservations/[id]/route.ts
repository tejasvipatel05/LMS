...
export const PATCH = requirePermission('approve_reservations')(async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    ...
    if (reservation.status !== 'PENDING') return NextResponse.json({ error: 'Only pending requests can be processed' }, { status: 400 })
    ...
    if (action === 'approve') {
      ...
      await prisma.borrowing.create({
        data: { bookId: reservation.bookId, userId: reservation.userId, dueDate, status: 'ACTIVE' }
      })

      await prisma.book.update({
        where: { id: reservation.bookId },
        data: { availableCopies: reservation.book.availableCopies - 1 }
      })

      await prisma.reservation.update({
        where: { id: reservationId },
        data: { status: 'FULFILLED', approvedBy: user.id, approvedAt: new Date(), notes: notes || null }
      })

      return NextResponse.json({ message: 'Borrow request approved successfully' })
    } else {
      // reject
      await prisma.reservation.update({
        where: { id: reservationId },
        data: { status: 'REJECTED', approvedBy: user.id, approvedAt: new Date(), notes: notes || null }
      })
      return NextResponse.json({ message: 'Borrow request rejected successfully' })
    }
  } catch (error) {
    console.error('Failed to process reservation:', error)
