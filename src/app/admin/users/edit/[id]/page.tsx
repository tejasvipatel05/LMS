...
export default function EditUserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentUser.email === user?.email && formData.role !== user.role) {
      if (!confirm("You are changing your own role. This may affect your access. Continue?")) {
        return
      }
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end space-x-4 pt-6 border-t">
            ...
            <Button 
              variant="primary" 
              type="submit" 
              disabled={saving || (formData.changePassword && formData.newPassword !== formData.confirmPassword)}
              className="min-w-32"
            >
              {saving ? (
